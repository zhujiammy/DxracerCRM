...
public class AddPublicCuePresenter implements AddPublicCueInterface.presenter,View.OnClickListener {

    private AddPublicCueInterface.View view;
    private AddPublicCueActivity addPublicCueActivity;
    ...
    public AddPublicCuePresenter(AddPublicCueInterface.View view, final AddPublicCueActivity addPublicCueActivity){
        this.addPublicCueActivity = addPublicCueActivity;
        this.view = view;
        addPublicCueActivity.leadGetDate.setOnClickListener(this);
        addPublicCueActivity.selectAddress.setOnClickListener(this);
        //获取当前日期
        getDate();
        addPublicCueActivity.leadGetDate.setText(mYear + "-" + (mMonth + 1) + "-" + mDay);


        addPublicCueActivity.popWindow = new PopWindow.Builder(addPublicCueActivity)
                .setStyle(PopWindow.PopWindowStyle.PopUp)
                .setView( addPublicCueActivity.customView)
                .create();

        addPublicCueActivity.citySelect1Activity=(CitySelect1Activity) addPublicCueActivity.customView.findViewById(R.id.apvAddress);
        addPublicCueActivity.citySelect1Activity.setOnAddressPickerSure(new CitySelect1Activity.OnAddressPickerSureListener() {
            @Override
            public void onSureClick(String Province, String City, String District, String ProvinceCode, String CityCode, String DistrictCode) {
                ...
                addPublicCueActivity.selectAddress.setText(Province+" "+City+" "+District);
                ProvinceCodestr = ProvinceCode;
                CityCodestr = CityCode;
                DistrictCodestr = DistrictCode;
                addPublicCueActivity.popWindow.dismiss();
            }
        });
                ...
                switch (msg.what) {
                    case 0:// 解析返回数据
                        ...
                        for (JsonElement accesschannels:jsonArray){
                            ...
                            arrayAdapter_lead_source = new ArrayAdapter<AccessChannelsModel>(addPublicCueActivity.getApplication(), R.layout.simple_spinner_item,accessChannelsModels_lead_source);
                            arrayAdapter_lead_source.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
                            addPublicCueActivity.Accesschannels.setAdapter(arrayAdapter_lead_source);
                            addPublicCueActivity.Accesschannels.setOnItemSelectedListener(Accesschannelslistener);
                        }

                        if(!addPublicCueActivity.type.equals("2")){
                            //获取渠道
                            int d=arrayAdapter_lead_source.getCount();
                            for(int j=0;j<d;j++){
                                if(addPublicCueActivity.intent.getStringExtra("leadSource").equals(arrayAdapter_lead_source.getItem(j).getKey())){
                                    addPublicCueActivity.Accesschannels.setAdapter(arrayAdapter_lead_source);
                                    addPublicCueActivity.Accesschannels.setSelection(j,true);
                                }
                            }
                        }
                    ...
                    case 1:// 解析返回数据
                        ...
                        for (JsonElement accesschannels:jsonArray_scale){
                            ...
                            arrayAdapter_customer_scale = new ArrayAdapter<AccessChannelsModel>(addPublicCueActivity.getApplication(), R.layout.simple_spinner_item,accessChannelsModels_customer_scale);
                            arrayAdapter_customer_scale.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
                            addPublicCueActivity.Customersize.setAdapter(arrayAdapter_customer_scale);
                            addPublicCueActivity.Customersize.setOnItemSelectedListener(Customersizelistener);
                        }
                        ...
                        if(!addPublicCueActivity.type.equals("2")){
                            ...
                            for(int j=0;j<a;j++){
                                if(addPublicCueActivity.intent.getStringExtra("customerScale").equals(arrayAdapter_customer_scale.getItem(j).getKey())){
                                    addPublicCueActivity.Customersize.setAdapter(arrayAdapter_customer_scale);
                                    addPublicCueActivity.Customersize.setSelection(j,true);
                                }
                            }
                        }
                    ...
                    case 2:// 解析返回数据
                        ...
                        for (JsonElement accesschannels:jsonArray_industry){
                            ...
                            arrayAdapter_customer_industry = new ArrayAdapter<AccessChannelsModel>(addPublicCueActivity.getApplication(), R.layout.simple_spinner_item,accessChannelsModels_customer_industry);
                            arrayAdapter_customer_industry.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
                            addPublicCueActivity.Customerindustry.setAdapter(arrayAdapter_customer_industry);
                            addPublicCueActivity.Customerindustry.setOnItemSelectedListener(Customerindustrylistener);
                        }
                        if(!addPublicCueActivity.type.equals("2")){
                            //客户行业
                            int c=arrayAdapter_customer_industry.getCount();
                            for(int j=0;j<c;j++){
                                if(addPublicCueActivity.intent.getStringExtra("customerIndustry").equals(arrayAdapter_customer_industry.getItem(j).getKey())){
                                    addPublicCueActivity.Customerindustry.setAdapter(arrayAdapter_customer_industry);
                                    addPublicCueActivity.Customerindustry.setSelection(j,true);
                                }
                            }
                        }
                    ...
                    case 3:
                        ...
                        if(code.equals("error")){
                            view.failed();
                            Toast.makeText(addPublicCueActivity,jsonObject.get("msg").getAsString(),Toast.LENGTH_SHORT).show();
                        }
    ...
    Spinner.OnItemSelectedListener Accesschannelslistener = new AdapterView.OnItemSelectedListener() {
        ...
        public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
            leadSource = ((AccessChannelsModel) addPublicCueActivity.Accesschannels.getSelectedItem()).getKey();
        }
    ...
    Spinner.OnItemSelectedListener Customersizelistener = new AdapterView.OnItemSelectedListener() {
        @Override
        public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
            customerScale = ((AccessChannelsModel) addPublicCueActivity.Customersize.getSelectedItem()).getKey();
        }
    ...
    Spinner.OnItemSelectedListener Customerindustrylistener = new AdapterView.OnItemSelectedListener() {
        @Override
        public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
            customerIndustry = ((AccessChannelsModel) addPublicCueActivity.Customerindustry.getSelectedItem()).getKey();
        }
    ...
    public void onClick(View v) {
        if(!addPublicCueActivity.type.equals("2")){
            ...
            try {
                Date date = sf.parse(addPublicCueActivity.leadGetDate.getText().toString());
        ...
        //获取日期
        if (v == addPublicCueActivity.leadGetDate){
            DatePickerDialog datePickerDialog = new DatePickerDialog(addPublicCueActivity,
                    ...
                    new DatePickerDialog.OnDateSetListener() {
                        ...
                        public void onDateSet(DatePicker view, int year, int month, int dayOfMonth) {
                            ...
                            try {
                                ...
                                addPublicCueActivity.leadGetDate.setText(sdate);
                            } catch (ParseException e) {
                                e.printStackTrace();
                            }
        ...
        //省市区选择
        if(v == addPublicCueActivity.selectAddress){
            addPublicCueActivity.popWindow.show();
        }
    }
    ...
    public void SaveData(){
        //构造请求参数
        Map<String, String> reqBody = new ConcurrentSkipListMap<>();
        reqBody.put("leadGetDate", addPublicCueActivity.leadGetDate.getText().toString());
        if(leadSource.equals("请选择")){
            Toast.makeText(addPublicCueActivity,"获取渠道不能为空！",Toast.LENGTH_SHORT).show();
        }else if(TextUtils.isEmpty(addPublicCueActivity.customerShortName.getText().toString())){
            Toast.makeText(addPublicCueActivity,"客户简称不能为空！",Toast.LENGTH_SHORT).show();
        }else  {
            reqBody.put("leadSource",leadSource);
            reqBody.put("customerShortName", addPublicCueActivity.customerShortName.getText().toString());
            reqBody.put("customerFullName", addPublicCueActivity.customerFullName.getText().toString());
            ...
            reqBody.put("customerAddress", addPublicCueActivity.customerAddress.getText().toString());
            reqBody.put("customerIntroduce", addPublicCueActivity.customerIntroduce.getText().toString());
